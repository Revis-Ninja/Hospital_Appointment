...
public class yuyueAction extends ActionSupport
{
	...
	public String yuyueYisheng()
	{
		Map session=ActionContext.getContext().getSession();
		TYisheng user=(TYisheng)session.get("yisheng");
		
		String sql="from TYuyue where yishengId="+user.getYishengId();
	...
	public String yuyueDelMy()
	{
		...
		this.setMessage("Cancel Successfully");
		this.setPath("yuyueManaMy.action");
		return "succeed";
	}
	public String yuyueDelYisheng()
	{
		yuyueDAO.delete(yuyueDAO.findById(id));
		this.setMessage("Cancel Successfully");
		this.setPath("yuyueYisheng.action");
		return "succeed";
	}
